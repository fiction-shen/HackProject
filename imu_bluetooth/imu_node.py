...
class AnyDevice(gatt.Device):
    ...
    def parse_imu(self,buf):
        ...
        if buf[0] == 0x11:
            ...
            if ((ctl & 0x0001) != 0):
                tmpX = ((buf[L+1]<<8) | buf[L]) * scaleAccel; L += 2 
                print("\taX: %.3f"%tmpX); # x加速度aX
                tmpY = ((buf[L+1]<<8) | buf[L]) * scaleAccel; L += 2 
                print("\taY: %.3f"%tmpY); # y加速度aY
                tmpZ = ((buf[L+1]<<8) | buf[L]) * scaleAccel; L += 2
            ...
            print(" ")
            if ((ctl & 0x0004) != 0):
                tmpX = ((buf[L+1]<<8) | buf[L]) * scaleAngleSpeed; L += 2 
                print("\tGX: %.3f"%tmpX) # x角速度GX
                tmpY = ((buf[L+1]<<8) | buf[L]) * scaleAngleSpeed; L += 2 
                print("\tGY: %.3f"%tmpY) # y角速度GY
                tmpZ = ((buf[L+1]<<8) | buf[L]) * scaleAngleSpeed; L += 2
            ...
            print(" ")
            if ((ctl & 0x0008) != 0):
                tmpX = ((buf[L+1]<<8) | buf[L]) * scaleMag; L += 2
                print("\tCX: %.3f"%tmpX); # x磁场CX
                tmpY = ((buf[L+1]<<8) | buf[L]) * scaleMag; L += 2
                print("\tCY: %.3f"%tmpY); # y磁场CY
                tmpZ = ((buf[L+1]<<8) | buf[L]) * scaleMag; L += 2
            ...
            print(" ")
            if ((ctl & 0x0020) != 0):
                tmpAbs = ((buf[L+1]<<8) | buf[L]) * scaleQuat; L += 2
                print("\tz: %.3f"%tmpAbs); # w
                tmpX =   ((buf[L+1]<<8) | buf[L]) * scaleQuat; L += 2
                print("\tw: %.3f"%tmpX); # x
                tmpY =   ((buf[L+1]<<8) | buf[L]) * scaleQuat; L += 2
                print("\tx: %.3f"%tmpY); # y
                tmpZ =   ((buf[L+1]<<8) | buf[L]) * scaleQuat; L += 2
                print("\ty: %.3f"%tmpZ); # z
            ...
            print(" ")
            if ((ctl & 0x0040) != 0):
                tmpX = ((buf[L+1]<<8) | buf[L]) * scaleAngle; L += 2
                print("\tangleX: %.3f"%tmpX); # x角速
                tmpY = ((buf[L+1]<<8) | buf[L]) * scaleAngle; L += 2
                print("\tangleY: %.3f"%tmpY); # y角速
                tmpZ = ((buf[L+1]<<8) | buf[L]) * scaleAngle; L += 2
                print("\tangleZ: %.3f"%tmpZ); # z角速
            ...
            print(" ")
            if ((ctl & 0x0080) != 0):
                tmpX = ((buf[L+1]<<8) | buf[L]) / 1000.0; L += 2
                print("\toffsetX: %.3f"%tmpX); # x坐标
                tmpY = ((buf[L+1]<<8) | buf[L]) / 1000.0; L += 2
                print("\toffsetY: %.3f"%tmpY); # y坐标
                tmpZ = ((buf[L+1]<<8) | buf[L]) / 1000.0; L += 2
            ...
            if ((ctl & 0x0400) != 0):
                ...
                print("\tadc: %u"%tmpU16); # 12位精度ADC的数值(0-4095),对应的电压量程由0x24指令设定
